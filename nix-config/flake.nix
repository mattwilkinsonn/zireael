{
  ...
  outputs =
    ...
    in
    {
      ...
      # as mattmini (jobs route to whichever agent is free), distinct
      # agent name + host tag. See darwin/dedicatedmacio-mini/system.nix
      # (SEA-840).
      #
      # Identical wiring to mattmini: STABLE nixpkgs + stable nix-darwin
      # + stable home-manager, pkgs constructed in the `let` block and
      # passed via specialArgs + extraSpecialArgs (the same stable-HM
      # eval-cycle short-circuit documented on the mattmini entry above).
