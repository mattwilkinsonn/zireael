...
# dedicatedmac.io), used as a STOPGAP macOS CI runner while the
# Buildkite-trial macOS minutes are exhausted and the owned mattmini
# isn't yet racked.
#
# Roles:
...
#      same as production.
#
# RENTAL posture — differs from the owned mattmini in two ways:
#   - This box lives in dedicatedmac.io's datacenter on a network we
#     don't control. "The LAN" here is the provider's shared subnet,
#     not a home network — so the pf agent-egress lockdown (below) is
#     even more warranted: the agent UID has no business reaching any
#     RFC1918 host on a rental subnet. Kept verbatim from mattmini.
#   - Secrets at rest are plaintext in /etc/buildkite-agent (macOS has
#     no systemd-creds; same tradeoff mattmini takes). The provider has
#     physical/root access, so the exit plan is: WIPE THE BOX before the
#     subscription expires (dedicatedmac re-images on return anyway).
#     That makes the agent token + CI App key unrecoverable on return,
#     so no rotation is needed — the wipe is the revocation. See the
#     INSTALL.md "Rental exit" section.
#
# Headless / always-on. Tailscale SSH (LAN-side native sshd is
# intentionally unloaded) is the only user-facing interface; the
# tailnet ACLs gate which devices can reach it (only the MBP). pmset is
# locked into "never sleep, never hibernate, autorestart on power loss"
# — see the pmset activation block below.
#
# RAM note: 16 GB unified memory (M4 base). A SINGLE agent instance
# runs here — one cold-cache `cargo build -j` on the workspace can
# spike several GB across codegen workers, and a single instance keeps
# the box inside 16 GB without swap pressure. Single instance also
# keeps sccache simple: with no concurrent agents, the agent's own
...
let
  ...
  mkAgentEnv = name: {
    ...
    # M4 base core count. The base M4 mini is 10-core: 4 performance +
    # 6 efficiency. CARGO_BUILD_JOBS is set to the performance-core
    # count to keep heavy parallel codegen on the P-cores (and bounded
    # for 16 GB RAM with a single agent). VERIFY ON BOX: `sysctl -n
    # hw.perflevel0.physicalcpu` (P-cores) — bump to match if this is an
    # M4 Pro variant (10-core M4 Pro is 8P+4E, 12-core is 8P+4E + more).
    CARGO_BUILD_JOBS = "4";
      ...
      ProgramArguments = [
        "${pkgs.writeShellScript "buildkite-agent-${name}-start" ''
          ...
          exec ${pkgs.buildkite-agent}/bin/buildkite-agent start \
            --name dedicatedmacio-mini-${name} \
            --tags queue=macos-arm64-selfhosted,os=macos,arch=arm64,host=dedicatedmacio-mini \
...

{
  ...
  networking.hostName = "dedicatedmacio-mini";
  networking.computerName = "dedicatedmacio-mini";
  networking.localHostName = "dedicatedmacio-mini";
  ...
  # --hostname=dedicatedmacio-mini` (bootstrap step 4). After that the
  ...
  # tailnet device at:
  #     https://dedicatedmacio-mini.tail08a5c5.ts.net:9443/
  #
  ...
  # devices can talk to dedicatedmacio-mini; we trust that as the auth layer.
