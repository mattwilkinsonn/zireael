...
# awsmac — AWS EC2 mac2-m2.metal instance (Apple M2, 8-core, 24 GB),
# used as a STOPGAP macOS CI runner while the Buildkite-trial macOS
# minutes are exhausted and neither the owned mattmini nor the
# dedicatedmac.io rental is up yet. The most disposable runner of the
# lot — billed by the hour on a $100 AWS-credit budget, terminated the
# moment a longer-lived box comes online.
#
# Roles:
#   1. Self-hosted Buildkite CI agent for the sealedsecurity org
#      (native macOS arm64, queue `macos-arm64-selfhosted` — the SAME
#      queue mattmini serves, so test-macos.yml needs no change; jobs
#      route to whichever agent is free). arm64 matches the seal user
#      majority + the release.yml arm64-macOS target, and current macOS
#      on supported hardware means the sandbox-exec tests behave the
#      same as production.
#
# EC2 posture — differs from the owned mattmini in three ways:
#   - Account is `ec2-user` (the AWS macOS AMI's default admin), NOT
#     `mattw` like the rest of the fleet. Skipping account creation is
#     one less manual step on a box that lives for days. home.nix +
#     the users block below are pinned to ec2-user accordingly.
#   - This box lives in AWS's network on a VPC subnet we don't own. The
#     pf agent-egress lockdown (below) cuts the agent UID off from the
#     VPC's RFC1918 range — even more warranted than on a home LAN.
#     Inbound is gated by Tailscale (native sshd disabled); the EC2
#     security group is a second layer but the tailnet ACLs are the
#     trust boundary.
#   - Secrets at rest are plaintext in /etc/buildkite-agent (macOS has
#     no systemd-creds; same tradeoff mattmini takes). The exit plan is
#     cleaner than a wipe: `aws ec2 terminate-instances` + release the
#     dedicated host destroys the volume and the secrets with it, so no
#     rotation is needed — termination is the revocation. See the
#     INSTALL.md "Teardown" section.
#
# Headless / always-on (for the instance's lifetime). Tailscale SSH
# (LAN-side native sshd is intentionally unloaded) is the only
# user-facing interface; the tailnet ACLs gate which devices can reach
# it (only the MBP). pmset is locked into "never sleep" — see the pmset
# activation block below (autorestart/hibernate are moot on a VM but
# kept harmless for config parity).
#
# RAM note: 24 GB on mac2-m2.metal. A SINGLE agent instance runs here —
# one cold-cache `cargo build -j` on the workspace can spike several GB
# across codegen workers; the extra headroom over the 16 GB minis is
# nice but a single instance keeps sccache race-free by construction,
# which matters more than squeezing in a second agent on a box that's
# alive for days.
...
let
  ...
  # ec2-user's own git@github.com: SSH clones to HTTPS+App-token too.
  ...
  mkAgentEnv = name: {
    ...
    # mac2-m2.metal core count. The M2 in mac2-m2.metal is 8-core:
    # 4 performance + 4 efficiency. CARGO_BUILD_JOBS is set to the
    # performance-core count to keep heavy parallel codegen on the
    # P-cores (24 GB RAM gives ample headroom for a single agent).
    # VERIFY ON BOX: `sysctl -n hw.perflevel0.physicalcpu` (P-cores).
    CARGO_BUILD_JOBS = "4";
  ...
  mkAgentDaemon = name: {
    serviceConfig = {
      ...
      ProgramArguments = [
        "${pkgs.writeShellScript "buildkite-agent-${name}-start" ''
          ...
          exec ${pkgs.buildkite-agent}/bin/buildkite-agent start \
            --name awsmac-${name} \
            --tags queue=macos-arm64-selfhosted,os=macos,arch=arm64,host=awsmac \
      ...
      EnvironmentVariables = {
        ...
        # processes, not ec2-user's own git. See agentGitConfig.
...

{
  ...
  networking.hostName = "awsmac";
  networking.computerName = "awsmac";
  networking.localHostName = "awsmac";

  # User registration. nix-darwin needs `users.users.<name>` declared
  # so home-manager.users.<name> resolves. On EC2 mac the AMI ships
  # with `ec2-user` (admin, passwordless sudo, our SSH keypair already
  # wired) — so unlike the rest of the fleet we apply home-manager to
  # ec2-user instead of creating a `mattw` account (one less manual
  # step on a box that lives for days). The account itself is
  # AMI-provisioned; this block just points nix-darwin at it.
  ...
  # --hostname=awsmac` (bootstrap step 4). After that the
  ...
  # tailnet device at:
  #     https://awsmac.tail08a5c5.ts.net:9443/
  #
  ...
  # devices can talk to awsmac; we trust that as the auth layer.
  ...
  environment.etc."pf.agent.conf".text = ''
    ...
    # Everything else (ec2-user's shell, system daemons, anyone not
  ...
  # — NOT /etc/gitconfig, so it doesn't rewrite ec2-user's own git. See the
