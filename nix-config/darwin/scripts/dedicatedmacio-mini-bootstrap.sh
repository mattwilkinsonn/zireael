#!/usr/bin/env bash
# First-boot bootstrap for dedicatedmacio-mini (RENTED Apple Silicon
# Mac mini, M4, from dedicatedmac.io — STOPGAP macOS CI runner).
#
# Pre-reqs (see darwin/dedicatedmacio-mini/INSTALL.md for the full procedure):
#   - macOS (current release) — as provisioned by dedicatedmac.io
#     (the box arrives with macOS already installed; no OCLP).
#   - An admin user. dedicatedmac provisions a provider-named admin
#     account; create / rename to `mattw` per INSTALL.md so the
#     home-manager user resolves (the system.nix expects `mattw`).
...
#     you have an SSH path, or scp it over).
#
...
#   2. macOS hostname set to `dedicatedmacio-mini` (must precede tailscale up
#      or your tailnet hostname will be `dedicatedmacio-mini-local` or similar)
...
# this host. dedicatedmacio-mini runs untrusted CI workflows via the
...
#
# Usage:
#   bash dedicatedmacio-mini-bootstrap.sh
#   bash dedicatedmacio-mini-bootstrap.sh --auth-key tskey-auth-...
#   TAILSCALE_AUTH_KEY=tskey-auth-... bash dedicatedmacio-mini-bootstrap.sh
...
TARGET_HOSTNAME="dedicatedmacio-mini"
TS_HOSTNAME="${TARGET_HOSTNAME}.tail08a5c5.ts.net"

require_non_root
parse_tailscale_auth_key "$@"
require_hostname dedicatedmacio-mini
...
# `tailscale up` picks up `dedicatedmacio-mini.local` (the LocalHostName /
...
# read it at launch (see darwin/dedicatedmacio-mini/system.nix). The decrypt-
...
if ! command -v darwin-rebuild >/dev/null 2>&1; then
	...
	echo "  nix run nix-darwin -- switch --flake .#dedicatedmacio-mini"
	sudo HOME="$HOME" nix run --extra-experimental-features 'nix-command flakes' \
		nix-darwin -- switch --flake "$NIX_CONFIG_DIR#dedicatedmacio-mini" --show-trace
else
	step "darwin-rebuild switch --flake .#dedicatedmacio-mini"
	sudo HOME="$HOME" darwin-rebuild switch \
		--flake "$NIX_CONFIG_DIR#dedicatedmacio-mini" \
		--show-trace
fi
...
if sudo launchctl list 2>/dev/null | grep -q com.sealedsecurity.glances; then
	echo "  glances: loaded (reachable at https://dedicatedmacio-mini.tail08a5c5.ts.net:9443/)"
else
