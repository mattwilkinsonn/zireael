...
- **Commit promptly — don't let uncommitted work pile up in a workspace.** A sibling or supervisor op can rewrite an ancestor of your `@` and rebase you at any time (a status tool that snapshots on a timer — jjui, `starship-jj` — does this constantly, and that's fine): *committed* work rebases cleanly, *uncommitted* edits are exactly what `update-stale` discards. In a shared workspace, **snapshot with `jj describe` after each edit pass — before you run lint or tests** — so the work is committed into `@` and rebases cleanly; don't batch several passes into one window. **Reserve `jj new` (stepping off to a fresh change) until that step's checks pass** — running it before verification lands your lint/test fixups in the new empty child instead of the change being verified.
...
- **Stale working copy while holding uncommitted edits.** If any `jj` command reports the working copy is stale (a sibling/supervisor op rewrote an ancestor of your `@`) and you have uncommitted edits, **copy the changed files aside before `jj workspace update-stale`** — the resync can divert or discard them. After resync, run `jj log -r 'divergent()'`: update-stale often *snapshots* your edits into a divergent commit sharing a `<change-id>` with another copy rather than discarding them. **Once a change is divergent its bare `<change-id>` is ambiguous** — `jj diff -r <change-id>`, `abandon`, etc. error out, so address each copy by its disambiguated `<change-id>/N` form (shown in the log) or its commit-id through every step. Inspect the snapshot (`jj diff -r <change-id>/N`), then either:
  - **Keep it:** `jj edit <change-id>/N` first — update-stale may have left `@` on the *updated sibling*, so without this later edits land on the wrong commit. Then describe it and move your bookmark onto it. It is already a commit; do **not** re-run `jj commit`.
  - **Discard it:** if you already restored from your aside copy, `jj abandon <change-id>/N` the duplicate, then commit your restored edits.
