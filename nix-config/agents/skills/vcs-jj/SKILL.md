...
- **Stale working copy while holding uncommitted edits.** If any `jj` command reports the working copy is stale (a sibling/supervisor op rewrote an ancestor of your `@`) and you have uncommitted edits, **copy the changed files aside before `jj workspace update-stale`** — the resync can divert or discard them. After resync, `jj log -r 'divergent()'`: update-stale often *snapshots* your edits into a divergent `<change-id>/N` commit rather than discarding them. Inspect it (`jj diff -r <id>`), then either keep it (describe + move your bookmark onto it) or, if you already restored from your copy, abandon the duplicate. Either way, commit the recovered edits immediately — the store is still churning.
