...
**Merge-ready ⟺ every review thread resolved.** No exceptions. Never report a PR merge-ready while any thread is unresolved — enumerate the unresolved threads (path, author, why) and drive the count to **zero** first. `is_outdated` and `is_resolved` are **distinct**: an outdated thread (line moved / file changed / rebased away) is still an **open conversation** until explicitly resolved — outdated is never "handled." A thread that is stale because its finding no longer exists (file/line removed or superseded) is a **pre-authorized bot-only resolve** under the cases above: post a one-line "obsolete, with the reason" reply and resolve it, no per-reply approval needed (same lane as a fixed-and-live bot thread). Never resolve a thread silently — every resolve carries a reply first.

**A thread is resolved only when it is fixed or properly deferred — never left open, never resolved bare.** Deferral is a strict ordered protocol: **(1) file the follow-up issue**, **(2) post a comment** on the thread explaining the deferral and citing that issue, **(3) resolve the thread**. All three, in order — the issue exists before the comment so the comment can reference it (resolving a thread adds no comment body of its own, so the reference must live in the step-2 comment). "Bucket-limited", "outdated", or "will do later" are reasons to *defer* (which means run the protocol), never to leave a thread open or to resolve it without the issue+comment. A deferral resolve still needs Matt's OK on the reply wording unless it falls in the pre-authorized bot-only cases above. If the GraphQL bucket blocks the resolve, the deferral is an **owed action you MUST carry** until the bucket recovers — it never falls off; the PR is not merge-ready until it's done.
