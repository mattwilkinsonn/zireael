...
- **A gated PR is not done — hold your lane until it merges.** "Done" = merged (or closed/dropped), never "green and waiting at the gate." An open PR can still bounce back to you — a bot re-review posts a P1/P2, a CI leg goes red, the human requests changes — and you must be present to auto-fix and re-drive it. So don't context-switch to new work, and don't volunteer for a new lane, while any PR you own can still bounce; only offer for new work when **every** lane you own is actually merged. This composes with `rule://never-block`: yield the turn while you wait (don't foreground-block), but the gated lane stays yours across those yields — the bounce or the merge wakes you back to it. Full rule: `rule://hold-your-lane`.
...
- **Route a broadcast by who must see it.** Anything **every** agent must receive — a posture change, a standing directive, wave-wide policy, a critical CI gotcha, a freeze — goes to **`#announcements`** (the one channel with universal subscription). **`#coordination.general` does NOT reach everyone** — agents receive a `#coordination.*` channel only after they `cotal_join` it, so a fleet-wide directive posted there is invisible to everyone not already in it. `#coordination.<issue>` is for active hand-off among the agents already on that issue; `#svc.<name>` for a service (@mention its owner); a DM for point-to-point. Full routing matrix: `skill://multi-agent-wave`.
- **Never block yourself — stay reachable.** Never sit in a foreground blocking wait on a peer or a job (`irc wait`, `irc send await:true`, a blocking `job poll`) as your idle/coordination pattern — it makes you deaf to steering, other peers, and reassignment until that one thing lands, and two agents blocking on each other **deadlock**. End your turn instead (the harness delivers peer messages + job completions into your next turn) or poll non-blockingly (`irc inbox` / `job list`) and yield. A backgrounded wait you launch and yield from (e.g. `wait-for-reviews` via `bash async:true`) is fine; a foreground one that holds the turn open is the banned thing. Full rule: `rule://never-block`.
...
- Rules (read `rule://<name>` when the work matches): `red-green-testing`, `planning-evidence`, `pre-finish-checks`, `commit-conventions`, `enumerate-pr-review-surfaces`, `never-block`, `hold-your-lane`. Always-applied: `process-safety`. Auto-firing (TTSR trigger on retry config/calls): `no-retries`.
