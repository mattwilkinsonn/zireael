#!/usr/bin/env bash
# One-time bootstrap on mattlinuxpro: encrypt the Buildkite agent token
...
# can decrypt. The trashcan has no hardware TPM, but the machine ID is
...
#
# Usage:  sudo bash nixos/scripts/mattlinuxpro-encrypt-agent-token.sh
...
# nixos/modules/buildkite-agent.nix).
...
echo "via the decrypt-agent-token.service oneshot (declared in the shared"
echo "nixos/modules/buildkite-agent.nix module). Restart to pick up a"
echo "rotation:"
